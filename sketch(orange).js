...
function preload(){
  orangeImg = loadImage('./assets/org.ico');
}
let heartArr = [];
let colorList = []; 
let lastTime = 0; // 마지막 실행 시간
let interval = 250; // n초마다 실행 (예: 3초 = 3000ms)
...
function draw() { 
  imageMode(CENTER); // 이미지를 중심 기준으로 배치
  image(orangeImg, mouseX, mouseY, 15, 15); // 오렌지 이미지 크기: 100x100
}

function MakeHeart(){
  imageMode(CENTER); // 이미지를 중심 기준으로 배치
  image(orangeImg, mouseX, mouseY, 100, 100); // 오렌지 이미지 크기: 100x100
}
